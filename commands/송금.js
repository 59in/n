...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (!user) return message.reply("**송금할 대상을 멘션해줘!**") // 멘션 한 유저가 없다면 리턴
        ...
        if (!sk) return message.reply("**가입을 먼저 진행해줘! ( ?돈줘 )**") // 데이터베이스에서 userid 가 메세지를 보낸사람의 id인것이 없다면 리턴
        if (!tkdeoqkd) return message.reply("**송금할 대상이 가입을 하지 않았어!**") // 위에꺼랑 비슷 (데이터베이스에서 멘션한 유저를 찾을 수 없으면 리턴)
        const betting = parseInt(args[1]) //2번쨰 args에서 숫자를 찾음
        if (!betting) return message.reply("**사용법 : !송금 @멘션 (금액)**") // 2번쨰 args에서 숫자를 못찾으면 리턴
        if (message.content.includes("-")) return message.reply("**금액엔 -가 포함되면 안돼!**") // 메세지 내용에 -가 포함되면 리턴
        const money = parseInt(sk.money) // money를 메세지를 보낸사람의 돈으로 지정
        const money2 = parseInt(tkdeoqkd.money) // money2를 멘션한 유저의 돈으로 지정
        if (money < betting) return message.reply("**가진 돈보다 많은 금액을 송금할 수 없어!**")
        if (message.author.id == user.id) return message.reply("**자신한텐 송금할 수 없어!**") // 맨션 한 유저가 메세지를 보낸유저와 아이디가 같다면 return
        ...
        const embed = new (require("discord.js")).MessageEmbed() //임베드 생성
            .setTitle("**송금이 정상적으로 처리되었어!**") // 임베드 제목
            .setColor("BLUE") // 임베드 색깔
